...
export default function Home() {
    ...
    return (
        <section className="overflow-auto h-screen w-full bg-[#121212] flex flex-col px-3">
            <Navbar />
            {payloadData.filter === '' ? (
                <div style={{ height: playingData.playing ? `${height - 70}px` : `${height}px`, transition: '0.5s', width: userData.user ? `${width - 70}px` : `${width}px`, marginLeft: userData.user ? '70px' : 0 }} className="ml-[6%] bg-[#1b1b1b] overflow-y-auto w-[94%] h-[85%] rounded-lg px-[1.5rem] py-[1rem]">
